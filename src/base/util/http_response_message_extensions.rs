mod http_response_message_extensions {
    ...
    pub fn add_call_time_headers(response: &mut Response, start: SystemTime, end: SystemTime) {
        let duration = end.duration_since(start).expect("end should be after start");
        response.headers_mut().insert(
            "X-CallSent-Timestamp",
            start.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs().into()
        );
        response.headers_mut().insert(
            "X-CallReceived-Timestamp",
            end.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs().into()
        );
        response.headers_mut().insert(
    ...
    pub fn get_call_length(response: &Response) -> Option<String> {
        response.headers().get("X-CallLength-Milliseconds")
    ...
    pub async fn process_string_response(response: Response, throw_exception_on_failure: bool) -> Result<String, Box<dyn std::error::Error>> {
        if response.status().is_success() {
            let result = response.text()?;
            // Log the successful request here using your preferred logging framework.
            Ok(result)
        } else if throw_exception_on_failure {
            Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "HTTP request failed")))
        } else {
            Ok(String::new())
        }
    }
